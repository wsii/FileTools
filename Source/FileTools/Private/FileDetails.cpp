...
void FFileDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	Category.AddCustomRow(FilterString)
		...
		[
			...
			.OnClicked_Raw(this, &FFileDetails::CreateFolder)
		];
	

	IDetailCategoryBuilder& Category1 = DetailBuilder.EditCategory("File Delete");
	TSharedPtr<IPropertyHandle> DeleteFilePathInput = Category1.AddProperty("DeleteFilePath").GetPropertyHandle();
	TSharedPtr<IPropertyHandle> DeleteFileNameInput = Category1.AddProperty("DeleteKeyWords").GetPropertyHandle();
	// DeleteFilePathInput.Get()->SetOnPropertyValueChanged(FSimpleDelegate::CreateRaw(this,&FFileDetails::CheckPath));

	Category1.AddProperty("DeleteFolder");
	Category1.AddProperty("DeleteFile");
	// Category1.AddProperty("DeleteInvert");
	
	Category1.AddCustomRow(FilterString)
	.WholeRowContent()
	[
		SNew(SSpacer)
	];
	...
	Category1.AddCustomRow(FilterString)
		...
		[
			SNew(SButton)
			// .ButtonStyle(FAppStyle::Get(),"FlatButton.Success")
			.Text(FText::FromString("File Delete")).HAlign(HAlign_Center)
			.OnClicked_Raw(this, &FFileDetails::DeleteFiles)
		];

	IDetailCategoryBuilder& Category2 = DetailBuilder.EditCategory("File Move");
	TSharedPtr<IPropertyHandle> MoveFileSourcePathInput = Category2.AddProperty("MoveFileSourcePath").GetPropertyHandle();
	TSharedPtr<IPropertyHandle> MoveFileTargetPathInput = Category2.AddProperty("MoveFileTargetPath").GetPropertyHandle();
	// MoveFileSourcePathInput.Get()->SetOnPropertyValueChanged(FSimpleDelegate::CreateRaw(this,&FFileDetails::CheckPath));
	
	TSharedPtr<IPropertyHandle> MoveFileTypeInput = Category2.AddProperty("MoveKeyWords").GetPropertyHandle();
	Category2.AddProperty("CopyFiles");
	Category2.AddCustomRow(FilterString)
	.WholeRowContent()
	[
		SNew(SSpacer)
	];
	...
	Category2.AddCustomRow(FilterString)
		...
		[
			...
			// .ButtonStyle(FAppStyle::Get(),"FlatButton.Success")
			.Text(FText::FromString("File Move")).HAlign(HAlign_Center)
			.OnClicked_Raw(this, &FFileDetails::MoveFiles)
		];
}
